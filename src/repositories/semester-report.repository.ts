...
import { CreateBatchSemesterReportDto, CreateSemesterReportDto } from "src/modules/semester-report/dto/create-semester-report.dto";
...
export class SemesterReportRepository extends Repository<SemesterReport> {
    ...
    async findAllReport() {
        const query = this.dataSource
            ...
            .leftJoinAndSelect('semesterReport.extracurricularScores', 'extracurricularScores')
            .leftJoinAndSelect('extracurricularScores.extracurricular', 'extracurricular')
        return await query.getMany();
    }

    async createSemesterReport(body: CreateSemesterReportDto) {
        const { absentDays, extracurricularScores, leaveDays, ranking, sickDays, scores, totalScore, semester, studentNationalId } = body
        const newSemesterReport = new SemesterReport();
        const queryRunner = this.dataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        const student: Student = await queryRunner.manager.findOne(Student, { where: { studentNationalId: studentNationalId }, select: { id: true, semesterReports: { id: true, semester: true } } });
        if (!student) {
        ...
        for (let index = 0; index < scores.length; index++) {
            ...
            const subject = await queryRunner.manager.findOne(Subject, { where: { name: score.subjectName } });
            if (!subject) {
                await queryRunner.rollbackTransaction();
        ...
        for (let index = 0; index < extracurricularScores.length; index++) {
            ...
            const extracurricular = await queryRunner.manager.findOne(Extracurricular, { where: { name: extracurricularScore.extracurricularName } });
            if (!extracurricular) {
                await queryRunner.rollbackTransaction();
    ...
    async createBatchSemesterReport(batch: CreateBatchSemesterReportDto) {
        ...
        for (let index = 0; index < body.length; index++) {
            ...
            const { absentDays, extracurricularScores, leaveDays, ranking, sickDays, scores, totalScore, studentNationalId } = element
            const student: Student = await queryRunner.manager.findOne(Student, { where: { studentNationalId: studentNationalId }, select: { id: true, semesterReports: { id: true, semester: true } }, relations: { semesterReports: true } });
            if (!student) {
                await queryRunner.rollbackTransaction();
                throw new NotFoundException('student not found');
            }
            ...
            for (let index = 0; index < scores.length; index++) {
                const score = scores[index];
                let subject = await queryRunner.manager.findOne(Subject, { where: { name: score.subjectName } });
                if (!subject) {
                    subject = new Subject();
            ...
            for (let index = 0; index < extracurricularScores.length; index++) {
                const extracurricularScore = extracurricularScores[index];
                let extracurricular = await queryRunner.manager.findOne(Extracurricular, { where: { name: extracurricularScore.extracurricularName } });
                if (!extracurricular) {
