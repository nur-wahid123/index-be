...
import { BatchLinkSubjectDto, LinkSubjectDto } from "src/modules/study-groups/dto/link-subject.dto";
...
export class StudyGroupRepository extends Repository<StudyGroup> {
    ...
    async detachSubject(subjectDto: LinkSubjectDto) {
        ...
        try {
            const studyGroup = await queryRunner.manager.findOne(StudyGroup, { where: { id: studyGroupId }, relations: ['subjects'] })
            if (!studyGroup) {
                throw new NotFoundException('study group not found')
            }
            const subject = await queryRunner.manager.findOne(Subject, { where: { id: subjectId } })
            if (!subject) {
                throw new NotFoundException('subject not found')
            }

            studyGroup.subjects = studyGroup.subjects.filter((subject) => subject.id !== subjectId)
    ...
    async linkBatchSubject(subjectDto: BatchLinkSubjectDto) {
        ...
        try {
            ...
            const subjects = await queryRunner.manager.find(Subject, { where: { id: In(subjectsId) } })
